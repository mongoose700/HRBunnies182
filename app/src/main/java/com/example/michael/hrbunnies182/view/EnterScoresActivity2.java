 ...
 */
public class EnterScoresActivity2 extends AppCompatActivity {

    private int MAP_LAYER = 0;
    private int TRAIN_LAYER = 1;
...
//        System.out.println("Adding an edge to the screen: " + edge);
        ...
        LayerDrawable layers = (LayerDrawable) mainView.getDrawable();

        BitmapDrawable mapDrawable = (BitmapDrawable) layers.getDrawable(0);
        BitmapDrawable trainDrawable = new BitmapDrawable(this.getBaseContext().getResources(),
                ((BitmapDrawable) layers.getDrawable(1)).getBitmap());
//        SurfaceView surfaceView = (SurfaceView) findViewById(R.id.surfaceView);

        int mapWidth = mapDrawable.getBitmap().getWidth();
        int mapHeight = mapDrawable.getBitmap().getHeight();

        Bitmap newBitmap = Bitmap.createBitmap(mapWidth, mapHeight,
                Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(newBitmap);
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(colorMap.get(curPlayer.getColor()));

        canvas.drawBitmap(trainDrawable.getBitmap(), 0, 0, new Paint());
...
//            System.out.println("Scaling by " + mapWidth / 624.0 + " and " + mapHeight / 417.0);
        ...
        BitmapDrawable transDrawable = new BitmapDrawable(this.getBaseContext().getResources(),
                newBitmap);

        layers.setDrawableByLayerId(TRAIN_LAYER, transDrawable);
        mainView.invalidateDrawable(layers);
    }
...
//            System.out.println("Scaling by " + mapWidth / 624.0 + " and " + mapHeight / 417.0);
                RectF trainCar = new RectF(
                        (float) (centerX - (4 * mapWidth / 624.0)),
                        (float) (centerY - (10 * mapHeight / 417.0)),
                        (float) (centerX + (4 * mapWidth / 624.0)),
                        (float) (centerY + (10 * mapHeight / 417.0)));
                canvas.save(Canvas.MATRIX_SAVE_FLAG);
                canvas.rotate((float) train.getTheta(), centerX, centerY);
...
//        System.out.println("Adjusted height scale: " + ((size.y - offset.y) / 417.0));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Create two layers, one for the map and one transparent
        Drawable mapDrawable = this.getBaseContext().getResources().getDrawable(R.drawable.ticket_to_ride);

        // Transparent
        BitmapDrawable transDrawable = new BitmapDrawable(this.getBaseContext().getResources(),
                makeTransparent(((BitmapDrawable) mapDrawable).getBitmap()));

        LayerDrawable layers = new LayerDrawable(new Drawable[] {mapDrawable, transDrawable});
        layers.setId(0, MAP_LAYER);
        layers.setId(1, TRAIN_LAYER);
        ...
        ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent results = new Intent(EnterScoresActivity2.this, com.example.michael.hrbunnies182.view.ResultsActivity.class);
    ...
    // Convert a Bitmap to be transparent
    // http://stackoverflow.com/questions/15084128/how-to-make-bitmap-transparent
